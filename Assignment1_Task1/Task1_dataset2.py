...
import math
from mpl_toolkits.mplot3d import Axes3D  # noqa: F401 unused import  	#For 3D plot in dataset2

ax = plt.axes(projection='3d')											#For 3D plot in dataset2
...
def load_dataset(data_file):
    ...
    return dataset
dataset = load_dataset("a1_d2.csv")
class0=[]
class1=[]
...
#05. Plot1: 3D Scattered points plot:  
ax.scatter3D(c0[:,0], c0[:,1], c0[:,2] ,color="green", label='class 0')		#For 3D plot in dataset2
ax.scatter3D(c1[:,0], c1[:,1], c1[:,2] ,color="red", label='class 1')		#For 3D plot in dataset2
plt.show()
